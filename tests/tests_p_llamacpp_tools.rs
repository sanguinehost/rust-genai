...
mod llamacpp_tools_tests {
	use genai::adapter::adapters::llamacpp::{
		...
		tool_parser::{contains_tool_calls, parse_tool_calls},
		tool_templates::{ToolFormat, apply_tool_template, detect_tool_config},
	};
...
This should return the current weather."#;
	...
	fn test_multiple_tool_schema() {
		...
		let time_tool = Tool::new("get_time").with_schema(json!({
			"type": "object",
			"properties": {
	...
	fn test_enum_schema_handling() {
		let tool = Tool::new("set_mode").with_schema(json!({
			"type": "object",
			"properties": {
				"mode": {
					"type": "string",
	...
	fn test_array_schema_handling() {
		let tool = Tool::new("process_list").with_schema(json!({
			"type": "object",
			"properties": {
				"items": {
					"type": "array",
