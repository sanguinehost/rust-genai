...
use super::tool_templates::{ToolConfig, ToolFormat};
use crate::chat::{MessageContent, ToolCall};
...
fn parse_generic_tools(text: &str) -> Result<Vec<ToolCall>> {
	...
	let re = Regex::new(r"```json\s*(.*?)\s*```").map_err(|e| Error::Internal(format!("Regex error: {}", e)))?;
...
fn fix_json_issues(json_str: &str) -> Result<String> {
	...
	let key_re = Regex::new(r"(\w+):").map_err(|e| Error::Internal(format!("Regex error: {}", e)))?;
