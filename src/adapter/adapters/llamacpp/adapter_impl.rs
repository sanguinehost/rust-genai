...
use crate::chat::{
	ChatMessage, ChatOptionsSet, ChatRequest, ChatResponse, ChatRole, ChatStreamResponse, MessageContent, Usage,
};
...
use super::model_manager::{ModelManager, resolve_model_path};
use super::schema_to_grammar::tools_to_gbnf;
use super::streamer::{StreamChunk, create_streaming_channel};
use super::tool_parser::{contains_tool_calls, parse_tool_calls};
use super::tool_templates::{apply_tool_template, detect_tool_config};
...
impl LlamaCppAdapter {
	...
	pub async fn exec_chat_stream_native(
		...
		tokio::task::spawn_blocking(move || {
			...
			match futures::executor::block_on(Self::generate_response_blocking_with_data(
				model_iden,
				base_url,
				chat_req_clone,
				generation_options,
	...
	async fn generate_response_blocking_with_data(
		...
		let result =
			Self::generate_with_model_path(&model_iden, model_path, chat_req, options, stream_tx.clone()).await;
	...
	fn create_regular_prompt(
		messages: &[ChatMessage],
		loaded_model: &super::model_manager::LoadedModel,
	) -> Result<String> {
			...
			if !tools.is_empty() && config.supports_tools {
				...
				match tools_to_gbnf(tools) {
					Ok(grammar) => {
						...
						if let Some(grammar_sampler) =
							LlamaSampler::grammar_lazy(model, &grammar, "root", &config.trigger_words, &trigger_tokens)
						{
