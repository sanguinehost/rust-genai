...
impl GrammarConverter {
	...
	fn convert_object(&mut self, obj: &serde_json::Map<String, Value>, rule_name: &str) -> Result<String> {
		let schema_type = obj.get("type").and_then(|v| v.as_str()).unwrap_or("object");
	...
	fn convert_object_type(&mut self, obj: &serde_json::Map<String, Value>, rule_name: &str) -> Result<String> {
		...
		self.rules
			.insert(rule_name.to_string(), format!("{} ::= {}", rule_name, object_rule));
...
mod tests {
	...
	fn test_tools_to_gbnf() {
		let tool = Tool::new("get_weather").with_schema(json!({
			"type": "object",
			"properties": {
				"city": {"type": "string"}
			},
