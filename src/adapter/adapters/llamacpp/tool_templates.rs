...
pub enum ToolFormat {
	...
	Llama3 { trigger_tag: String },
	/// Functionary v3 style
	Functionary { function_tag: String },
	/// Hermes 2 Pro style
	Hermes,
	/// Generic JSON style
	Generic,
...
pub fn apply_tool_template(messages: &[ChatMessage], tools: &[Tool], config: &ToolConfig) -> Result<String> {
	match &config.format {
...
fn apply_llama3_template(messages: &[ChatMessage], tools: &[Tool], trigger_tag: &str) -> Result<String> {
	...
	for message in messages {
		match message.role {
			ChatRole::System => {
				if tools.is_empty() {
					// Only add if we didn't add tools system message
